...
(function (factory) {
    ...
    const requestIt = (url, data, callback) => {
        const deJson = (data) => data;
        const deCode = callback;
        if (!callback) callback = 'deJson';
        else callback = 'deCode';
        data.callback = callback;
        data._ = 1619337230942;
        if (debuging) console.log("url", url, 'data', data);
        url = urlEncode(url, data);
        if (debuging) console.log('encoder', url);
    ...
    $.getChallenge = (url, data, callback) => {
        url = url + "/cgi-bin/get_challenge";
        requestIt(url, data, callback);
    }
    ...
    function info(d, k) {
        ...
        // return "{SRBX1}" + new Buffer.from(xEncode(json(d), k)).toString('base64');
        // return "{SRBX1}" + new Buffer.from(xEncode(json(d), k), 'base64').toString();
    }
    ...
    function chksum(d) {
        // const sha1 = () => {
        //     CryptoJS.HmacSHA1(str, key).toString();
        // };
        return sha1(d);
    }
